...
class UtilisateurSerializer(serializers.ModelSerializer):
    """Lecture seule — informations de base"""
    ...
    class Meta:
        ...
        fields = [
            ...
            'permission_2fa', 'compte_verrouille', 'date_inscription',
            'derniere_connexion',
        ]
...
class UtilisateurCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'password', 'password2', 'telephone', 'adresse',
            'wilaya', 'role',
        ]
...
class ParentCreateSerializer(serializers.Serializer):
    """Créer un parent + son compte utilisateur en une seule requête"""
    # Données utilisateur
    ...
    # Données parent
    relation_enfant   = serializers.ChoiceField(
    ...
    def create(self, validated_data):
        ...
        parent = Parent.objects.create(
            user=user,
            relation_enfant=validated_data.get('relation_enfant'),
...
class EnseignantCreateSerializer(serializers.Serializer):
    """Créer un enseignant + son compte utilisateur"""
    # Données utilisateur
    email           = serializers.EmailField()
    first_name      = serializers.CharField(max_length=50)
    last_name       = serializers.CharField(max_length=50)
    ...
    niveaux             = serializers.CharField(max_length=50, help_text="Ex: A1,A2,B1")
...
class EtudiantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Etudiant
        fields = [
            'id', 'user', 'date_naissance', 'genre', 'parent', 'parent_nom',
            'groupe', 'groupe_nom', 'niveau_actuel', 'moyenne_generale',
            'taux_assiduité', 'statut_etudiant', 'date_inscription',
        ]
...
class EtudiantCreateSerializer(serializers.Serializer):
    """Créer un étudiant + son compte utilisateur"""
    # Données utilisateur
    email           = serializers.EmailField()
    first_name      = serializers.CharField(max_length=50)
    last_name       = serializers.CharField(max_length=50)
    ...
    id_parent       = serializers.IntegerField(required=False, allow_null=True)
    id_groupe       = serializers.IntegerField(required=False, allow_null=True)
    niveau_initial  = serializers.ChoiceField(
        choices=['A1', 'A2', 'B1', 'B2', 'C1'], default='A1'
    )

    def validate_id_groupe(self, value):
        if value:
            try:
                groupe = Groupe.objects.get(pk=value)
    ...
    def create(self, validated_data):
        ...
        etudiant = Etudiant.objects.create(
            ...
            parent_id=validated_data.get('id_parent'),
    ...
    def validate(self, attrs):
        ...
        if qs_salle.exists():
            raise serializers.ValidationError(
                f"La salle {salle} est déjà occupée à ce créneau."
            )
        ...
        if enseignant:
            ...
            if qs_ens.exists():
                raise serializers.ValidationError(
                    "L'enseignant a déjà un cours à ce créneau."
                )
        return attrs
...
class NoteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'note_obtenue', 'note_max', 'pourcentage', 'niveau_attribue',
            'remarque_prof', 'statut_passage', 'date_saisie',
        ]
...
class AbsenceSerializer(serializers.ModelSerializer):
    ...
    def get_seance_date(self, obj):
        return str(obj.seance.date_seance)
...
class RessourceCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Get enseignant from request user
        request = self.context.get('request')
        try:
            enseignant = request.user.enseignant_profile
        except Enseignant.DoesNotExist:
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'expediteur', 'expediteur_nom', 'destinataire', 'destinataire_nom',
                  'sujet', 'contenu', 'date_envoi', 'lu', 'pieces_jointes']
...
class PieceJointeSerializer(serializers.ModelSerializer):
    class Meta:
        model = PieceJointe
        fields = ['id', 'nom_fichier', 'chemin_fichier', 'taille_fichier',
                  'type_fichier', 'date_upload']
